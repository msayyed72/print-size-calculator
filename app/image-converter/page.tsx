...
export default function ImageConverter(){
 const[file,setFile]=useState<File|null>(null);const[format,setFormat]=useState("image/webp");const[quality,setQuality]=useState("0.9");const[out,setOut]=useState("");const[outSize,setOutSize]=useState(0);const[error,setError]=useState("");const[drag,setDrag]=useState(false);const inputRef=useRef<HTMLInputElement>(null);
 const sameFormat=!!file&&file.type===format;
 useEffect(()=>()=>{if(out)URL.revokeObjectURL(out)},[out]);
 const choose=(f:File|null)=>{if(!f)return;setFile(f);setOut("");setOutSize(0);setError("");const preferred=f.type==="image/png"?"image/webp":f.type==="image/webp"?"image/jpeg":"image/webp";setFormat(preferred)};
 const convert=()=>{if(!file)return;if(sameFormat){setError(`This image is already ${formats.find(f=>f.value===format)?.name}. Choose another format.`);return}setError("");const img=new Image();const url=URL.createObjectURL(file);img.onload=()=>{const c=document.createElement("canvas");c.width=img.naturalWidth;c.height=img.naturalHeight;const ctx=c.getContext("2d");if(!ctx){URL.revokeObjectURL(url);setError("Canvas is not available in this browser.");return}ctx.drawImage(img,0,0);c.toBlob(blob=>{URL.revokeObjectURL(url);if(!blob){setError("This image could not be converted.");return}const next=URL.createObjectURL(blob);setOut(next);setOutSize(blob.size)},format,format==="image/png"?undefined:Number(quality))};img.onerror=()=>{URL.revokeObjectURL(url);setError("Unsupported or invalid image file.")};img.src=url};
  ...
  <section className={styles.hero}><div className={styles.eyebrow}>IMAGE TOOL</div><h1>Convert images<br/><em>without the clutter.</em></h1><p className={styles.intro}>Change PNG, JPEG and WebP in seconds. Everything runs locally in your browser.</p>
   <div className={styles.workspace}>
    <div className={styles.card}>
     ...
     {file&&<div className={styles.fileCard}><img className={styles.preview} src={URL.createObjectURL(file)} alt="Selected image preview"/><div><div className={styles.fileName}>{file.name}</div><div className={styles.fileMeta}>{pretty(file.size)} · {file.type.replace("image/","").toUpperCase()} · {file.width?`${file.width} × ${file.height}`:"Image"}</div></div><button className={styles.change} onClick={()=>inputRef.current?.click()}>Change</button></div>}
    </div>
    <div className={styles.card}><div className={styles.controls}>
      <div><div className={styles.label}>OUTPUT FORMAT</div><div className={styles.formatGrid}>{formats.map(f=><button key={f.value} className={`${styles.format} ${format===f.value?styles.formatActive:""} ${file&&f.value===file.type?styles.formatDisabled:""}`} disabled={!!file&&f.value===file.type} onClick={()=>setFormat(f.value)}><strong>{f.name}</strong><small>{f.note}</small></button>)}</div></div>
      ...
      <button className={styles.button} onClick={convert} disabled={!file||sameFormat}>{sameFormat?"Choose a different format":"Convert image"}</button>
